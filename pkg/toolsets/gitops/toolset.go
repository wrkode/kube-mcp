...
// Toolset implements the GitOps toolset for Flux and Argo CD.
type Toolset struct {
...
// NewToolset creates a new GitOps toolset with CRD detection.
func NewToolset(provider kubernetes.ClientProvider, discovery *kubernetes.CRDDiscovery) *Toolset {
	enabled := false
	var kustomizationGVR, helmReleaseGVR, applicationGVR schema.GroupVersionResource
	hasKustomization, hasHelmRelease, hasApplication := false, false, false

	if discovery != nil {
		// Check for Flux Kustomization
		if gvr, ok := discovery.GetGVR(KustomizationGVK); ok {
			enabled = true
			hasKustomization = true
			kustomizationGVR = gvr
		}

		// Check for Flux HelmRelease
		if gvr, ok := discovery.GetGVR(HelmReleaseGVK); ok {
			enabled = true
			hasHelmRelease = true
			helmReleaseGVR = gvr
		}
		...
		if gvr, ok := discovery.GetGVR(ApplicationGVK); ok {
			enabled = true
			hasApplication = true
			applicationGVR = gvr
		}
	...
	return &Toolset{
		...
		helmReleaseGVR:   helmReleaseGVR,
		applicationGVR:   applicationGVR,
		hasKustomization: hasKustomization,
		hasHelmRelease:   hasHelmRelease,
...
// IsEnabled returns whether the GitOps toolset is enabled.
